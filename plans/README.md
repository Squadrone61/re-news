...
| Prod image channel (plan 8) | `docker-compose.prod.yml` overrides `build:` with `!reset null` + `pull_policy: always`, pinning to `ghcr.io/squadrone61/re-news-{web,worker}:latest`. Rollback is a temporary tag swap to `:sha-<oldsha>`. Only `web` + `worker` carry the Watchtower label; `renews_db` never auto-updates. | Home server already runs Watchtower — the deploy surface shrinks to "push to main". Data containers must stay stable across rolls. |
| Backup service isolation (plan 8) | `docker-compose.backup.yml` with `profiles: [backup]` holds a one-shot `postgres:16-alpine` service; `make backup` (or host crontab) calls `docker compose … run --rm backup`. Dumps go to `./data/backups/YYYY-MM-DD.sql.gz`; `find -mtime +14 -delete` in the same entrypoint prunes old backups. | Keeps steady-state container count at 3. Out-of-band crontab means backups fire whether or not the worker is healthy. |
| `./data` mount into `web` (plan 8) | Compose mounts `./data:/app/data:ro` on `web` so the Settings page can read the worker-written `account_info.json`. Worker still has RW via `./data:/app/data`. | Avoids a worker-→web control channel just to expose auth state; cheap file IPC is fine at this scale. |
| Account-info capture (plan 8) | `accountInfo.ts` probes a small allowlist (`getAccountInfo` / `accountInfo` / `AccountInfo`) against `@anthropic-ai/claude-agent-sdk`, writing whatever fields it finds plus a `refreshedAt` stamp. Refreshes every 5 min. Web treats a missing file as "auth unknown" and a file `>10 min` stale as "stale". | The SDK's account-info API has moved across versions; this best-effort probe keeps the UI useful without hard-coding a specific export. |
| Token/cost accumulator (plan 8) | `pipeline/usage.ts` exposes a mutable `UsageTotals` threaded into `runResearch` / `runSummary`. Each SDK message is scanned for `usage` / `model_usage` / `message.usage` and cost fields; never throws. Persist happens after email-send, before the final `success` update. | Single accumulator keeps "how many tokens did this run use" coherent across both stages. On re-run-stage-2 only Stage 2's usage is counted against the new row (by design). |
| Cleanup retention (plan 8) | Fixed daily cron `0 3 * * *`, 30 days on `runs.finished_at`. Only deletes `/app/data/runs/<id>` — DB rows and `renderedOutput` remain. Runs without `finishedAt` are ineligible. | User-facing history is preserved; disk reclaim only touches per-run scratch. |
| Cron preview collision window (plan 8) | Collision = another enabled job whose `nextFireAt` is within 60 min *and* lands on the same UTC minute as the first upcoming fire of this schedule. Self-exclusion via `excludeId=<jobId>`. | Avoids false positives where two jobs share a minute but are weeks apart. The staggering advice (`:03`/`:17`/`:37`) targets LAN-scale collisions in the immediate future. |
