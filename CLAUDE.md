...
**Source descriptors are a tagged union.** `SourceInput` accepts either `{url, hint?, needsBrowser?}` or `{search, hint?}`. URL sources flow straight to a `research-source` Task; search sources are expanded to 3–5 URLs by the conductor via Claude's built-in `WebSearch` tool (subscription, no external API). The web `JobForm` exposes a per-row Type selector — never collapse the union back into a single shape; downstream code keys on the presence of `url` vs `search`.

**Fetch errors are surfaced, not hidden.** `research_raw.fetch_errors` is grouped by code via `summarizeFetchErrors` (in `@renews/shared`) and shown as a one-line banner on the run-detail page when count > 0. The same breakdown is also logged to `run_logs` (`sys`/`warn`) by `pipeline/research.ts#persist`. Don't drop entries from `fetch_errors` to "clean up" output — the visibility is the feature.
...
**Settings PUT preserves the password.** Empty string, literal `"***"`, or `undefined` for `gmailAppPassword` (and the same rule for `telegramBotToken`) means **no change** — only a new non-empty non-mask string writes it. Prevents accidental wipe on GET → form-submit.

**Delivery channel is per-job (email | telegram).** `Job.deliveryChannel` discriminates; `recipientEmail` is required iff `email`, `telegramChatId/Type/Title` are populated by the link flow iff `telegram`. The dispatcher is `pipeline/delivery.ts#runDelivery`; never call `runEmail` directly from the orchestrator. Preflight defers (not skips) with `telegram bot token missing` / `telegram not linked` so a misconfig is visible in the runs list. `runRender` still produces markdown/HTML/JSON into `Run.renderedOutput` for the UI preview and email body — Telegram bypasses that column and re-renders fresh chunks via `renderTelegramHtml` from `@renews/shared` (so web's resend route can reuse the same renderer without importing the worker package).

**Telegram = Bot API, long-poll.** Worker spawns one `getUpdates` long-poll loop (`telegram/poll.ts`, 25s timeout) alongside the 5s DB tick. Bots **cannot DM users who haven't messaged them first**, so linking goes through one-time `JobLinkToken` rows: web mints a token, UI surfaces a `t.me/<bot>?start=<token>` deep link (DM) or `/link <token>` instruction (group); when the bot sees the matching `/start` or `/link` command it sets `Job.telegramChatId/Type/Title`. Tokens live 10 min. `Setting.telegramBotUsername` is cached from `getMe` and **nulled on every token rotation** so the worker re-fetches; the link-mint API 400s with "Bot username not validated yet" until the worker fills it in. `Setting.telegramUpdateOffset` is the getUpdates cursor; advance only after successful batch processing. Self-heals: missing bot token → sleep+retry (no crash); 401 from the API → same. 429 `retry_after` becomes a sleep in the poll loop and a `RateLimitError` (→ `deferred`, `nextRunAt=resetAt`, no attempt bump) in the send path. Send errors rethrow as `telegram send: <reason>` and become `status='failed'`. Telegram only accepts the HTML subset `<b><i><u><s><code><pre><a>`; `renderTelegramHtml` escapes everything else and chunks at item boundaries to fit the 4096-char/message limit. Failure notices to the admin still go via email regardless of channel — admins always want email.

**Resend is channel-aware.** `POST /api/runs/:id/resend` reads `job.deliveryChannel`. Email path uses `web/src/lib/mailer.ts#sendConfiguredMail`; Telegram path uses `web/src/lib/telegram.ts#sendConfiguredTelegram` (web also reads `telegramBotToken` from settings — Settings UI is the source of truth, no env var). Stage 2 JSON drives both: Telegram re-renders fresh chunks from `stage2_json`; it does NOT read `renderedOutput`.
...
- `run_logs` — BigInt id, stage ∈ `research|summary|email|telegram|sys`
- `job_link_tokens` — one-time tokens for the Telegram DM/group linking flow; FK to `jobs`, 10-min `expires_at`
- `settings` adds `telegram_bot_token` / `telegram_bot_username` / `telegram_update_offset`; `jobs` adds `delivery_channel` (`email`|`telegram`), nullable `recipient_email`, and `telegram_chat_id`/`type`/`title`
