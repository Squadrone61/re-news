...
**Two-stage pipeline split is load-bearing.** Stage 1 (Sonnet, tools, `maxTurns: 40`, `permissionMode: 'acceptEdits'`) writes `research.json` capped at 25 items × 800 chars (prompt-enforced + post-parse truncation). Stage 2 (Haiku, no tools, `maxTurns: 1`) emits strict JSON validated by `StageTwoSchema` + `validateLengths` (subject ≤70, body ≤50 words, items ≤ `maxItems`). One retry on failure with a tighter prompt, then fail.
